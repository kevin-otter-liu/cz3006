...
def stats():
    df= pd.read_csv('network_data.csv', index_col=False,names=['type','flow_agent_addr',
    ...
    nx.draw_networkx(G, node_size=size, node_color=range(len(nodes)), width=weights, cmap=plt.cm.bwr)
